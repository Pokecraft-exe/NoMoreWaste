...
func rechercherAdherent(w http.ResponseWriter, r *http.Request) {
	...
	if q := r.URL.Query().Get("q"); q != "" {
		query += " where personne.nom ilike $" + strconv.Itoa(argPos) + " or personne.prenom ilike $" + strconv.Itoa(argPos) + " or personne.email ilike $" + strconv.Itoa(argPos)
		args = append(args, "%"+q+"%")
		argPos++
	}
	query += " order by adherent.adherent_id offset $" + strconv.Itoa(argPos) + " limit $" + strconv.Itoa(argPos+1)
	...
	for rows.Next() {
		var adherentID int
		var nom, prenom, email, statut, dateInscription string
		if err := rows.Scan(&adherentID, &nom, &prenom, &email, &statut, &dateInscription); err != nil {
			writeAPIError(w, http.StatusInternalServerError, "internal_server_error", "Failed to scan adherents")
			return
		}
		adherents = append(adherents, map[string]any{
			"adherent_id":      adherentID,
			"nom":              nom,
			"prenom":           prenom,
			"email":            email,
