...
func listerPlanningService(w http.ResponseWriter, r *http.Request) {
	...
	query := `select planning_service.planning_service_id, planning_service.service_id, service.libelle, planning_service.date_service,
	...
	for rows.Next() {
		var id, serviceID, capacite int
		var libelle, dateService, heureDebut, heureFin, statut string
		if err := rows.Scan(&id, &serviceID, &libelle, &dateService, &heureDebut, &heureFin, &capacite, &statut); err != nil {
...
func listerInscriptionsService(w http.ResponseWriter, r *http.Request, token *IntrospectionPayload) {
	...
	// No planning_service_id: an adherent listing "Mes inscriptions" - every
	// service slot they're registered for.
	if planningServiceID == "" {
		if !isAdherent(token) {
			writeAPIError(w, http.StatusForbidden, "access_denied", "Only an adherent can list their own registrations, or staff a slot's registrations via planning_service_id")
			return
		}
		adherentID, err := ownAdherentID(ctx, conn, token.CompteID)
		if err != nil {
		...
		rows, err := conn.Query(
			ctx,
			`select inscription_service.inscription_service_id, inscription_service.planning_service_id, inscription_service.statut,
				service.libelle, planning_service.date_service, planning_service.heure_debut, planning_service.heure_fin
			from inscription_service
			inner join planning_service on planning_service.planning_service_id = inscription_service.planning_service_id
			inner join service on service.service_id = planning_service.service_id
			where inscription_service.adherent_id = $1 and inscription_service.statut != 'annule'
			order by planning_service.date_service asc`,
			adherentID,
		)
		if err != nil {
		...
		for rows.Next() {
			var id, planningID int
			var statut, libelle, dateService, heureDebut, heureFin string
			if err := rows.Scan(&id, &planningID, &statut, &libelle, &dateService, &heureDebut, &heureFin); err != nil {
				writeAPIError(w, http.StatusInternalServerError, "internal_server_error", "Failed to scan registrations")
				return
			}
			inscriptions = append(inscriptions, map[string]any{
				"inscription_service_id": id, "planning_service_id": planningID, "statut": statut,
				"service": libelle, "date_service": dateService, "heure_debut": heureDebut, "heure_fin": heureFin,
			})
		}

		writeJSON(w, http.StatusOK, map[string]any{"total": len(inscriptions), "inscriptions": inscriptions})
		return
	}
