...
func listerRessourcesCuisine(w http.ResponseWriter, r *http.Request) {
	...
	if q := r.URL.Query().Get("q"); q != "" {
		conditions = append(conditions, "(titre ilike $"+strconv.Itoa(argPos)+" or $"+strconv.Itoa(argPos)+" ilike any(ingredients))")
		args = append(args, "%"+q+"%")
		argPos++
	}
	if nom := r.URL.Query().Get("nom"); nom != "" {
		conditions = append(conditions, "titre ilike $"+strconv.Itoa(argPos))
		args = append(args, "%"+nom+"%")
		argPos++
	}
	if ingredient := r.URL.Query().Get("ingredient"); ingredient != "" {
		conditions = append(conditions, "exists (select 1 from unnest(ingredients) i where i ilike $"+strconv.Itoa(argPos)+")")
		args = append(args, "%"+ingredient+"%")
		argPos++
	}
	if outil := r.URL.Query().Get("outil"); outil != "" {
		conditions = append(conditions, "exists (select 1 from unnest(outils) o where o ilike $"+strconv.Itoa(argPos)+")")
		args = append(args, "%"+outil+"%")
		argPos++
	}
