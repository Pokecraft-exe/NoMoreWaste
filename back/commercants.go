...
// rappelAdhesionCommercantHandler implements the commercant side of
// "prevoir un systeme de rappel automatique de renouvellement": staff
// triggers a reminder, delivered as a notification (visible in the
// commercant's notification bell) rather than through a separate tracking
// table like rappel_adhesion (which is adherent-specific).
func rappelAdhesionCommercantHandler(w http.ResponseWriter, r *http.Request) {
	token := tryAuth(w, r)
	if !token.Active {
		return
	}
	...
	if commercantIDParam == "" || message == "" {
		writeAPIError(w, http.StatusBadRequest, "invalid_request", "commercant_id and message are required")
		return
	}
	commercantID, err := strconv.Atoi(commercantIDParam)
	if err != nil {
	...
	if err := conn.QueryRow(
		ctx,
		"select compte.compte_id from commercant inner join compte on compte.personne_id = commercant.personne_id where commercant.commercant_id=$1",
		commercantID,
	).Scan(&compteID); err != nil {
		writeAPIError(w, http.StatusNotFound, "not_found", "Commercant not found")
		return
	}

	notifierCompte(ctx, conn, compteID, "rappel_adhesion", message, "")

	writeJSON(w, http.StatusCreated, map[string]any{"message": "Reminder sent successfully"})
}
