...
func getAnnonce(w http.ResponseWriter, r *http.Request, token *IntrospectionPayload) {
	...
	query := `select annonce_echange.annonce_echange_id, annonce_echange.compte_id, personne.nom, personne.prenom,
		categorie, titre, description, prix, statut, date_publication::text
		from annonce_echange
		inner join compte on compte.compte_id = annonce_echange.compte_id
		inner join personne on personne.personne_id = compte.personne_id`
	...
	if q := r.URL.Query().Get("q"); q != "" {
		conditions = append(conditions, "(titre ilike $"+strconv.Itoa(argPos)+" or description ilike $"+strconv.Itoa(argPos)+")")
		args = append(args, "%"+q+"%")
		argPos++
	}
	// auteur_id lets a caller list their own annonces (including cloturee
	// ones, for self-management from the profil page) - restricted to your
	// own compte_id unless you're staff, since cloturee annonces are
	// otherwise hidden from the public listing.
	mine := false
	if auteurParam := r.URL.Query().Get("auteur_id"); auteurParam != "" {
		if auteurID, convErr := strconv.Atoi(auteurParam); convErr == nil && (auteurID == token.CompteID || isStaff(token)) {
			conditions = append(conditions, "compte_id = $"+strconv.Itoa(argPos))
			args = append(args, auteurID)
			argPos++
			mine = true
		}
	}
	// statut lets staff (moderating from the back-office) filter on a
	// specific etat, cloturee included.
	statutFilter := false
	if statut := r.URL.Query().Get("statut"); statut != "" && isStaff(token) {
		conditions = append(conditions, "statut = $"+strconv.Itoa(argPos))
		args = append(args, statut)
		argPos++
		statutFilter = true
	}
	if !mine && !statutFilter {
	...
	for rows.Next() {
		...
		var nom, prenom, categorie, titre, description, statut, datePublication string
		var prix *float64
		if err := rows.Scan(&id, &auteurID, &nom, &prenom, &categorie, &titre, &description, &prix, &statut, &datePublication); err != nil {
...
func modifierAnnonce(w http.ResponseWriter, r *http.Request, token *IntrospectionPayload) {
	...
	if r.Form != nil && r.Form.Has("prix") {
		updates = append(updates, "prix=nullif($"+strconv.Itoa(argIndex)+",'')::numeric")
		args = append(args, r.FormValue("prix"))
		argIndex++
	}
	...
	args = append(args, annonceID)
	if _, err := conn.Exec(ctx, "update annonce_echange set "+strings.Join(updates, ", ")+" where annonce_echange_id=$"+strconv.Itoa(argIndex), args...); err != nil {
		writeAPIError(w, http.StatusBadRequest, "invalid_request", "Invalid field value")
		return
	}
