...
func listerThreads(w http.ResponseWriter, r *http.Request) {
	...
	query := `select forum_thread.forum_thread_id, personne.nom, personne.prenom, forum_thread.titre, forum_thread.message,
	...
	if q := r.URL.Query().Get("q"); q != "" {
		query += " where forum_thread.titre ilike $" + strconv.Itoa(argPos)
		args = append(args, "%"+q+"%")
		argPos++
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &nom, &prenom, &titre, &message, &vues, &dateCreation); err != nil {
